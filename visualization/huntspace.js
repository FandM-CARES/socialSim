// class that defines the huntspace element
import * as d3 from "d3";
export default class Huntspace {
  // Huntspace setup
  ...
  constructor(doc, result) {
    ...
    // TODO: look at staghunt file to set these dimensions
    ...
    this.characters = []; // not expected to change after initial load
    this.data; // not expected to change after initial load
    ...
    // Convert data into states for each character
    // --- STATES INIT START ---
  	result["states"].forEach((configFileState) => {
  		var state = [];
    ...
    // --- CHARACTER SPACE INIT START ---
    this.huntspace.append("g")
    	.attr("stroke-width", 1.5)
    	.attr("font-family", "sans-serif")
    	.attr("font-size", cellWidth/3.)
    	.attr("class", "characters");
    // --- CHARACTER SPACE INIT END ---

    // --- DRAW SPACE INIT START ---
  	var staghunt = this.data;
  ...
  // Moves characters to their next position
  drawCharacters() {
  	// draw character text
  	this.huntspace.selectAll(".characters")
  		.selectAll("text")
  		.data(this.characters[this.stateCounter % this.stateLength], d => d.id)
  		.join("text")
  		.text(d => d.id)
  		.style("fill", "red")
  		.style("font-size", "12px")
  		.style("dominant-baseline", function(d) {
  			return this.labelOffset[d.id][3];
  		})
  		.style("text-anchor", function(d) {
  			return this.labelOffset[d.id][2];
  		})
  		.transition().duration(function() {
  			return (this.stateCounter) ? 1000 : 0;
  		})
  		.attr("x", function(d) {
  			return scale(d.x + this.labelOffset[d.id][0]);
  		})
  		.attr("y", function(d) { return scale(d.y + this.labelOffset[d.id][1]); });
  	...
  	this.huntspace.selectAll(".characters")
  		...
  		.attr("d", function (d) {
  			return this.dLookup[d.type];
  		});
  }

  // Externally referenced methods (button behavior)
  prevData() {
  	if (stateCounter > 0) {
  		stateCounter--;
  		drawCharacters();
